...
(define-for-syntax allocator-setup-error-msg
  "Mutator must start with an 'allocator-setup' expression, such as: (allocator-setup <collector-module-path> (<cache-module-path> <cache-size> <set-size> <block-size>) <literal-number>)")

(define-syntax (mutator-module-begin stx) ;hier
  (syntax-case stx (allocator-setup)
    [(_ (allocator-setup . setup) module-expr ...)
     (begin
       (syntax-case #'setup ()
         [(collector heap-size cache)                  ;aangepast
          (begin
            (unless (module-path? (syntax->datum #'collector))
              (raise-syntax-error 'allocator-setup "expected a module path" #'collector))
            (unless (module-path? (syntax->datum #'cache))
              (raise-syntax-error 'allocator-setup "expected a module path" #'cache))
