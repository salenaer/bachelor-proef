...
(define (write-block-back read-adress write-adress)
  (define (loop ctr)
    (unless (= ctr cache:block-size)
      (child-write! (+ ctr write-adress) (cache-read (+ ctr read-adress)))
      (loop (+ ctr 1))))
  (loop 0))
...
(define (cache:before-eviction block-idx block-number source-adress)
  ...
  (vector-set! dirty-bits false)) ;eviction gebeurt alleen als data wordt overschreven => nieuwe data is clean
